package tech.whocodes.jampack.InteractionApi.Collections.Queries;

import tech.whocodes.jampack.InteractionApi.Collections.ETileItem;
import tech.whocodes.jampack.InteractionApi.InteractionApiPlugin;
import tech.whocodes.jampack.InteractionApi.PathFinding.GlobalCollisionMap;
...
public class TileItemQuery {
    ...
    public TileItemQuery withName(String name) {
        ...
        {
            try {
                return InteractionApiPlugin.itemDefs.get(tileItem.tileItem.getId()).getName().equals(name);
    ...
    public TileItemQuery nameContains(String name) {
        ...
        {
            try {
                return InteractionApiPlugin.itemDefs.get(tileItem.tileItem.getId()).getName().contains(name);
            } catch (ExecutionException e) {
                throw new RuntimeException(e);
            }
        }).collect(Collectors.toList());
                        ...
                        filter(item ->
                        {
                            try {
                                return WildcardMatcher.matches(input.toLowerCase(),
                                        Text.removeTags(InteractionApiPlugin.itemDefs.get(item.tileItem.getId()).getName().toLowerCase()));
                            } catch (ExecutionException e) {
                                throw new RuntimeException(e);
                            }
                        }).
    ...
    public boolean isNoted(ETileItem item) {
        ItemComposition itemComposition = InteractionApiPlugin.itemDefs.get(item.tileItem.getId());
