...
public class PrayerInteraction {
    ...
    public static void togglePrayer(Prayer a){
        if(InteractionApiPlugin.getClient().getVarbitValue(a.getVarbit())==0){
            InteractionApiPlugin.getClient().setVarbit(a.getVarbit(),1);
        }else{
            InteractionApiPlugin.getClient().setVarbit(a.getVarbit(),0);
        }
    ...
    public static void setPrayerState(Prayer prayer,boolean on){
        if(InteractionApiPlugin.getClient().isPrayerActive(prayer) != on){
            togglePrayer(prayer);
        }
    }
